...
all_file=glob.glob('../normal_data/*.h5ad')
FAILED=open('failed_normal.txt','w')
for the_file in all_file:
    try:
        ...
        obs_path = '../normal_processed/%s_obs.csv' % bname
        # %% for single %
        chunk_path_prefix = '../normal_processed/%s_' % bname
